...
import { Skeleton, Tab, TabContent, TabList } from "@/components/ui/misc";
        ...
        <TabContent value="model">
          <div className="mb-3 rounded-xl border border-hairline bg-surface-2/60 p-3">
            <p className="text-[12.5px] leading-relaxed text-muted-foreground">
              Skills run with or without a model. When a provider is configured, the model
              rewrites the briefing from the collected evidence — it is instructed never
              to add facts of its own. Without one, the deterministic analyst write-up is
              used, which is fully functional.
            </p>
          </div>
          <ul className="space-y-2">
                  ...
                  {capabilities.manifest.map((skill) => (
                    ...
                    >
                      ...
                      <div className="flex shrink-0 items-center gap-1">
                        <Badge tone={skill.runtime === "live" ? "info" : "neutral"} mono>
                          {skill.runtime}
                        </Badge>
